...
def load_audio_files(directory, sr=16000):
    ...
    labels = []
    class_labels = {
        label: i for i, label in enumerate(os.listdir(directory))
    ...
    for label in os.listdir(directory):
        ...
        if os.path.isdir(class_dir) and label != ".DS_Store":
            for file in os.listdir(class_dir):
                ...
                try:
                    ...
                    if audio is None or len(audio) == 0:
                        print(f"Skipping empty/zero-length audio: {file_path}")
                        continue
    ...
    if len(audio_data) == 0:
        raise RuntimeError(f"No audio files loaded from {directory} — check the path and file extensions")
...
def prepare_data(flatten_features, flatten_labels):
    X_train, X_val, y_train, y_val = train_test_split(
        flatten_features, flatten_labels, test_size=0.2, random_state=42
    )
